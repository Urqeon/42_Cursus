...
void Notice::notice(Client* client, const std::vector<std::string> commandParts, Server* srv)
{
    Client *targetClient = NULL;
    
    if (commandParts.size() < 2)
        return (client->sendReply(ERR_NEEDMOREPARAMS(client->getNickname(), "NOTICE")));
    ...
    if (commandParts.size() < 3)
        ...
        message = srv->merge_privmsg(std::vector<std::string>(commandParts.begin() + 2, commandParts.end()), " ");

    if (client->getNickname() != srv->getBot()->getBotnick())
        message = srv->getBot()->processMessage(client->getNickname() + "!" + message);
        
    if((commandParts[0] == "NOTICE"  || commandParts[0] == "/NOTICE") && commandParts.at(1)[0] != '#')
    {
        ...
        if (targetClient == NULL)
            ...
            targetClient->sendMessage(":" + client->getNickname() + " NOTICE " + commandParts.at(1) + " :" + message);
    }
