...
class Invite
{
    private:
		...
		~Invite();
	public:
        static void invite(Client* client, const std::vector<std::string> commandParts, Server* srv);
};
...
class Mode
{
	private:
		...
		~Mode();
	public:
    	static void mode(Client* client, const std::vector<std::string> commandParts, Server* srv);
};
